...
class BertLayerNorm(nn.Module):
    def __init__(self, hidden_size, eps=1e-5):
        ...
        self.weight = nn.Parameter(torch.ones(hidden_size))  # 并不会完全等于0 【768】
        self.bias = nn.Parameter(torch.zeros(hidden_size))
        self.variance_epsilon = eps

    def forward(self, x):
        u = x.mean(-1, keepdim=True)   # torch.Size([1, 157, 1])
        s = (x - u).pow(2).mean(-1, keepdim=True)    # torch.Size([1, 157, 1])
        x = (x - u) / torch.sqrt(s + self.variance_epsilon)
        return self.weight * x + self.bias  # torch.Size([1, 157, 768])
...
class BertSelfAttention(nn.Module):
    ...
    def transpose_for_scores(self, x, mask_qkv=None):
        "拆分 为 多个 head"
        if self.num_qkv > 1:  # 多组 multi head
    ...
    def forward(self, hidden_states, attention_mask, history_states=None,
        ...
        if history_states is None:
            ...
            mixed_key_layer = F.linear(hidden_states, self.key.weight)  # same
            mixed_value_layer = self.value(hidden_states)  # same
        else:
        ...
        query_layer = self.transpose_for_scores(mixed_query_layer, mask_qkv)  # torch.Size([1, 12, 157, 64])
        key_layer = self.transpose_for_scores(mixed_key_layer, mask_qkv)      # torch.Size([1, 12, 157, 64])
        value_layer = self.transpose_for_scores(mixed_value_layer, mask_qkv)  # torch.Size([1, 12, 157, 64])
        ...
        attention_scores = attention_scores + attention_mask  # torch.Size([1, 12, 157, 157])

        # Normalize the attention scores to probabilities.
        attention_probs = nn.Softmax(dim=-1)(attention_scores)   # torch.Size([1, 12, 157, 157])
        ...
        context_layer = torch.matmul(attention_probs, value_layer)  # torch.Size([1, 12, 157, 64])
...
class BertAttention(nn.Module):
    ...
    def forward(self, input_tensor, attention_mask, history_states=None,
                mask_qkv=None, seg_ids=None, key_history=None, value_history=None):
        """
        hidden: torch.Size([1, 157, 768])
        mask: torch.Size([1, 1, 157, 157])
        """
