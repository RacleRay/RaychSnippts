...
__all__ = ['youdao_translate', 'tencent_translate',
           'sogou_translate', 'baidu_translate', 'google_translate', 'LANGS']
...
def youdao_translate(content, from_lang='zh', to='en'):
    ...
    ret = requests.post('http://fanyi.youdao.com/translate_o',
                        data=data, headers=headers, timeout=10).json()
    trans = []
...
def tencent_translate(content, from_lang='zh', to='en'):
    ...
    ret = requests.post('https://fanyi.qq.com/api/translate',
                        data=data, headers=t_headers, timeout=10).json()
    trans = []
...
def sogou_translate(content, from_lang='zh-CHS', to='en'):
    ...
    ret = requests.post(
        'http://fanyi.sogou.com/reventondc/translate', data=data, timeout=10).json()
...
def baidu_translate(content, from_lang='zh', to='en'):
    ...
    ret = requests.post('https://fanyi.baidu.com/v2transapi',
                        data=data, headers=b_headers, timeout=10).json()
...
def google_translate(content, from_lang='zh-CN', to='en'):
    ...
    ret = requests.get(
        "https://translate.google.cn/translate_a/single", params=data, timeout=10).json()
    trans = []
    for item in ret[0]:
        trans.append(item[0])
    return ''.join(trans)
