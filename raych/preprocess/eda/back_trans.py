...
from back_trans_api import *

TRANS_APIS = [youdao_translate, sogou_translate,
              baidu_translate, google_translate, tencent_translate]
...
def main():
    parser = argparse.ArgumentParser(
        formatter_class=argparse.ArgumentDefaultsHelpFormatter, description='translation')
    parser.register("type", "bool", lambda v: v.lower() == "true")
    parser.add_argument("--match", type='bool', nargs="?",
                        const=True, default=False, help="Whether need to match api")
    ...
    parser.add_argument("--start", type=int, default=0, help="start index. 当输入start=-1，end=-1时，data中的全部数据进行翻译。在将en转回到zh时使用。")
    parser.add_argument("--end", type=int, default=0, help="end index. 当输入start=-1，end=-1时，data中的全部数据进行翻译。在将en转回到zh时使用。")
    ...
    # 当输入start=-1，end=-1时，data中的全部数据进行翻译。在将en转回到zh时使用。
    if flags.start != -1 and flags.end != -1:
        data = data[flags.start:flags.end]
    print("# Translation from {0} to {1}".format(flags.source, flags.target))
    print("# Start to translation file {0} from index {1} to {2}, size={3}".format(
        flags.data_file, flags.start, flags.end, flags.end-flags.start))
    ...
    with open(flags.out_file, 'w', encoding='utf8') as f:
        for i, item in enumerate(data):
            ...
            new_content, api_name = translation(
                content, flags.source, flags.target, old_api_name)
            end = time.time()
            if new_content is None:
                print("# No result, stop early")
                break
            print("# Finishing processing {0} with api {1} in {2:.2f} seconds, total time {3:.2f} min...".format(
                i, api_name, (end-start), (end-start_trans_time)/60))
